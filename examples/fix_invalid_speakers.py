...
def _get_speakers_by_oid(ngc: NudgisClient) -> dict[str, list[Speaker]]:
    speakers = {}
    ...
    for key in ['channels', 'videos', 'lives', 'photos']:
        for obj in catalog.get(key, []):
            ...
            if len({
                ...
                logger.error(
                    'Media "%s" will be ignored because its speakers '
                    'cannot be parsed reliably: %s',
                    oid, [speakers_emails, speakers_ids, speakers_names]
                )
                continue
            speakers[oid] = [
...
def _list_invalid_speakers(
    ...
    speakers_by_oid = _get_speakers_by_oid(ngc)
    valid_emails = {
        email: user
        for user in _get_users(ngc)
        if (email := (user.get('email') or '').strip())
    }

    speaker_data_by_email: dict[str, CSVSpeakerData] = {}
    for speakers in speakers_by_oid.values():
    ...
    for oid, current_speakers in speakers_by_oid.items():
        ...
        for speaker in current_speakers:
            if speaker.email not in corrections:
                logger.debug('"%s" not in CSV. Skipping...', speaker.email)
            ...
            if (
                ...
                logger.debug('No corrections for "%s". Skipping...', speaker.email)
                corrected_speakers.append(speaker)
                continue
        ...
        if current_speakers != corrected_speakers:
            ...
            if len(emails) > len(set(emails)) or len(ids) > len(set(ids)):
                logger.error(
                    ...
                    'on media "%s": current_speakers=%s / corrected_speakers=%s. '
                    ...
                    oid, current_speakers, corrected_speakers
                )
            else:
    ...
    for oid, corrected_speakers in corrected_speakers_by_oid.items():
        ...
        if apply:
            if oid[0] == 'c':
                ...
                payload['oid'] = oid
                url = 'medias/edit/'
            ngc.api(url, method='post', data=payload)
            logger.info(
                'Media "%s" has been updated: old_speakers=%s, new_speakers=%s',
                oid, speakers_by_oid[oid], corrected_speakers
            )
        else:
            logger.info(
                '[Dry-run] Media "%s" would have been updated: old_speakers=%s, new_speakers=%s',
                oid, speakers_by_oid[oid], corrected_speakers
            )
...
def fix_invalid_speakers(sys_args):
    ...
    if args.action == 'list':
        _list_invalid_speakers(ngc, args.csv_file, args.name_format, args.ignore_errors)
    elif args.action == 'fix':
        if args.apply:
            answer = input(
                'The script is running in normal mode. Changes from '
                f'"{args.csv_file}" will be applied to the {ngc.conf["SERVER_URL"]} database.\n'
                'Proceed ? [y / n]'
            )
        ...
        _fix_invalid_speakers(ngc, args.csv_file, args.apply)
