...
def find_user_by_email(ngc, email):
    response = ngc.api(
        'users/',
        params={
            'search': email,
...
def grant_personal_channel_permission(ngc, user_id, email):
...
def get_or_create_personal_channel_root(ngc, user_id, email):
    try:
        response = ngc.api('channels/personal/', params={'id': user_id})
        return response.get('oid')
    except Exception as e:
        if getattr(e, 'status_code', None) == 403:
            grant_personal_channel_permission(ngc, user_id, email)
            response = ngc.api('channels/personal/', params={'id': user_id})
            return response.get('oid')
        else:
...
def get_or_create_subchannel(ngc, title, parent_oid):
    try:
        ...
        if getattr(e, 'status_code', None) == 404:
            ...
            response = ngc.api('/channels/add/', method='post', data={'title': title, 'parent': parent_oid})
            return response.get('oid')
        else:
...
def get_personal_subchannel_oid(ngc, speaker, subchannel_title, apply=False):
    speaker_email = speaker.get('email')
    if not speaker_email:
        print(f'No email for speaker {speaker}, skipping personal channel.')
        return None

    users = find_user_by_email(ngc, speaker_email)
    if not users:
    ...
    root_oid = get_or_create_personal_channel_root(ngc, user_id, speaker_email)
    if not root_oid:
        return None
    print(f'Found personal channel root: {root_oid}')

    print(f'Fetching personal subchannel "{subchannel_title}" under channel {root_oid}')
    subchannel_oid = get_or_create_subchannel(ngc, subchannel_title, root_oid)
...
def main():
    ...
    ngc_dst = NudgisClient(args.conf_dst)
    dst_domain = ngc_dst.conf['SERVER_URL'].split('/')[2]
    print(f'> {dst_domain}\n')
    ...
    catalog = ngc_dst.get_catalog(fmt='flat')
    dst_external_refs = {}
