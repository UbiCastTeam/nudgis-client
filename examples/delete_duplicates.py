#!/usr/bin/env python3
"""
Script to detect and remove duplicate videos on a Nudgis portal.
"""
...
def process_channel(ngc, channel_info, enable_delete=False, videos_reference=None):
    ...
    for entry in channel_items.get('channels', []):
        process_channel(ngc, entry, enable_delete=enable_delete, videos_reference=videos_reference)
                ...
                if (
                    ...
                    if enable_delete:
                        try:
                            ...
                            print('Media %s have been deleted successfully.' % video_title)
                else:
...
def find_duplicate(ngc, channel_oid, enable_delete=False):
    ...
    process_channel(ngc, channel_parent['info'], enable_delete=enable_delete)
    return 0
...
if __name__ == '__main__':
    ...
    rc = find_duplicate(ngc, args.channel_oid, args.enable_delete)
    sys.exit(rc)
