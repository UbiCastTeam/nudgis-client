...
def _delete_medias(
    ...
    if apply:
        ...
        if permanent:
            params['force'] = 'yes'
        deleted_statuses = ngc.api('catalog/bulk-delete/', method='post', data=params)['statuses']
...
def delete_medias_from_csv(sys_args):
    ...
    # Ping
    print(f'Server url: {ngc.conf["SERVER_URL"]}')
    print(f'Nudgis version: {ngc.api("/")["mediaserver"]}')
    oids = [
    ...
    _delete_medias(ngc, oids, permanent=args.permanent, apply=args.apply)
