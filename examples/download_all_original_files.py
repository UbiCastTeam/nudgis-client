#!/usr/bin/env python3
"""
Script to download all best quality video files from a Nudgis portal.
"""
import argparse
import os
import re
...
def download_all_original_files(ngc, dir_path='videos'):
    ...
    while more:
        ...
        response = ngc.api('latest/', params=dict(start=start, content='v', count=20))
        for item in response['items']:
            index += 1
            print('// Media %s' % index)
            resources = ngc.api('medias/resources-list/', params=dict(oid=item['oid']))['resources']
            ...
            for r in resources:
                if r['format'] not in ('m3u8', 'youtube', 'embed'):
                    best_quality = r
                    break
            video_page = ngc.conf['SERVER_URL'] + '/permalink/' + item['oid'] + '/'
            if not best_quality:
...
if __name__ == '__main__':
    ...
    download_all_original_files(ngc, dir_path)
