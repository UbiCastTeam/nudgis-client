...
8. refresh the resources of the media and of all the media sharing the same resource files
   (``medias/resources-info`` gives the list of these media), because they all reference the
   files that have just been replaced.
...
def process_media(
    ...
    # The resources of a media can be shared with other media (duplicated media for example).
    # These media are impacted by the replacement and their resources are refreshed at the end.
    shared_with = get_sharing(ngc, oid)
    if shared_with:
        summary.warning(
            'Media %s: The resources are shared with %s other media (%s), '
            'these media are also impacted by the replacement.',
            oid, len(shared_with), ', '.join(shared_with),
        )
    ...
    if not apply:
        ...
        detail = f'{len(to_delete)} resource(s) to delete.'
        if shared_with:
            detail += f' Resources shared with: {", ".join(shared_with)}.'
    ...
    # The media sharing the same resources are refreshed too because they reference them as well.
    for impacted_oid in [oid, *shared_with]:
        try:
            ngc.api('medias/resources-check/', method='post', data=dict(oid=impacted_oid), timeout=180)
        except NudgisRequestError as err:
            summary.warning(
                'Media %s: Failed to refresh the resources of the media "%s": %s',
                oid, impacted_oid, str(err).strip(),
            )
        else:
            logger.info('Media %s: Resources of the media "%s" refreshed.', oid, impacted_oid)

    detail = f'{len(to_delete)} resource(s) deleted.'
    if shared_with:
        detail += f' Resources shared with: {", ".join(shared_with)}.'
    return 'done', detail
