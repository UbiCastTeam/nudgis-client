...
try:
    from nudgisclient.lib.utils import format_bytes, format_time
...
class Stats:
    ...
    def read_csv(self):
        ...
        logging.info('Found %s media, skipped %s', len(self.media_list), self.skipped)
    ...
    def filter_by_date(self, row):
        ...
        if self.options['start_date'] and creation < self.options['start_date']:
            logging.debug(
                'Skipping %s because it was created before the start date', row
            )
            self.skipped += 1
            return
        if self.options['end_date'] and creation > self.options['end_date']:
            self.skipped += 1
            logging.debug('Skipping %s because it was created after the end date', row)
            return
        ...
        for media in self.media_list:
            ...
            try:
                ...
                if origin.startswith('miris-box-') or origin.startswith('easycast-'):
                    ...
                    logging.warning('Unsupported origin "%s" for %s', origin, media)
                ...
                logging.warning('Failed to analyze origin for %s: %s', media, e)
