...
def upload_hls_files(ngc, uid, tmp_path):
    logger.info('Starting HLS upload (#%s).', uid)
    ngc.hls_upload(tmp_path + '.m3u8', f'speed-test-{uid}')


def upload_chunked_files(ngc, uid, files_list):
    logger.info('Starting chunked upload (#%s).', uid)
    for i, path in enumerate(files_list):
        logger.info('Process #%s, file %s/%s', uid, i + 1, len(files_list))
        ngc.chunked_upload(file_path=path, remote_path=f'speed-test-{uid}/' + os.path.basename(path))
...
def run_test(args):
    ...
    if args.chunk:
        ngc.conf['UPLOAD_CHUNK_SIZE'] = args.chunk * 1000
    ngc.check_server()
    ...
    files_list = []
    if args.count > 1:
        logger.info('Copying file %s times in "%s".', args.count - 1, tmp_path)
    ...
    for i in range(1, args.processes + 1):
        if args.m3u8:
            args_list.append((ngc, i, tmp_path))
        else:
            args_list.append((ngc, i, files_list))
    end = time.time()
    duration = end - start
    duration = round(duration, 2)
    logger.info('Initialisation done in %s s.', duration)
    ...
    duration = round(duration, 2)
    logger.info('Number of files uploaded: %s.', total_files)
    logger.info('Total size: %s MB.', total_size)
    logger.info('Average speed: %s kB/s.', avg_speed)
    logger.info('Upload duration: %s s.', duration)
    ...
    if not args.bench:
        ...
        for test_type in test_types:
            ...
            for entry in test_data:
                ...
                logger.info('\033[94m-- Step %s/%s Running with %s...\033[0m', len(results) + 1, steps, params)
        ...
        logger.info('CSV report will be written in "%s".', csv)
