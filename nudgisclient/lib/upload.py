...
def chunked_upload(
        ...
        while chunk:
            ...
            try:
                ...
                if offset == end_offset + 1 and 'upload_id' in data:
                    ...
                    logger.info('Offset issue detected during upload, ignoring error: %s', err)
                else:
                    raise
    ...
    logger.info('Upload finished, average bandwidth was %s/s.', format_bytes(bandwidth))
...
def hls_upload(
    ...
    if client.get_server_version() < (8, 2):
        raise RuntimeError('The HLS upload is not supported by the server.')
    ...
    for ts_path in ts_fragments:
        if not ts_path.is_file():
            logger.warning(
                'Found an element which is not a file in the ts fragments dir "%s". '
        ...
        if files_size > max_size or len(files_list) >= max_files:
            # Send files in list
            logger.info(
                'Uploading %s files (%.2f MB, only fragments) of "%s" in one request.',
                len(files_list), files_size / 1_000_000, ts_dir
            )
    ...
    logger.info(
        'Uploading %s files (%.2f MB, fragments and the playlist) of "%s" in one request.',
        len(files_list), files_size / 1_000_000, ts_dir
    )
    total_size += files_size
    data = dict(dir_name=remote_dir, hls_name=ts_dir.name)
    ...
    bandwidth = total_size / (time.time() - begin)
    logger.info(
        'Upload finished (%s files in "%s"), average bandwidth: %s/s',
        total_files_count, remote_dir, format_bytes(bandwidth)
    )
