"""
Nudgis client configuration library
...
def load_conf(default_conf: Path | str | dict | None = None, local_conf: Path | str | dict | None = None) -> dict:
    ...
    for conf_override in (default_conf, local_conf):
        ...
        if isinstance(conf_override, str):
            ...
            if conf_override.exists():
                ...
                logger.debug('Config file %s does not exist.', conf_override)
...
def check_conf(conf: dict) -> None:
    ...
    if not conf.get('SERVER_URL') or conf['SERVER_URL'] == 'https://nudgis':
...
def get_conf_for_unix_user(user: str) -> dict:
    ...
    settings_path = Path(f'/var/lib/ubicast-mediaserver/portals/{user}/private/mssettings.py')
    if not settings_path.exists():
        raise ConfigurationError(f'Instance settings file "{settings_path}" does not exists.')
    logger.info('Retrieving configuration from user "%s" instance.', user)
    ...
    logger.info('Site URL from settings: "%s".', site_url)
