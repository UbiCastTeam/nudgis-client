...
class NudgisRequestError(Exception):
...
class NudgisClient():
    ...
    RequestError: type[NudgisRequestError] = NudgisRequestError
    ...
    def get_server_version(self) -> tuple[int, ...]:
        if not hasattr(self, '_server_version'):
            try:
                ...
                # The "mediaserver" (Nudgis internal name) key was added in version 6.6.0
                version_str = response.get('mediaserver') or '6.5.4'
                self._server_version = tuple([int(i) for i in version_str.split('.')])
            except Exception as err:
                raise NudgisRequestError(
                    f'Failed to get Nudgis version: {err}',
                    status_code=getattr(err, 'status_code', None),
                    error_code=getattr(err, 'error_code', None)
                ) from err
            else:
                logger.debug('Nudgis version is: %s', self._server_version)
    ...
    def api(
        ...
        **kwargs
        ...
        logger.debug('API call duration: %.2f s - %s.', time.time() - begin, uri)
        return result
