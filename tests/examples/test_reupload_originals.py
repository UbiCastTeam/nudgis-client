...
def test_process_media_refreshes_the_media_sharing_the_resources(tmp_path):
    video = write(tmp_path / 'fa5381aa.mp4', b'x' * 100)
    client = make_client(
        resources_list=[
            [resource('old_360.mp4')],
        ...
        sharing=[{'oid': 'v1', 'title': 'Test'}, {'oid': 'v2', 'title': 'Other'}, {'oid': 'v3'}],
    )
    summary = ro.Summary()
    ...
    assert status == 'done'
    assert detail == '1 resource(s) deleted. Resources shared with: v2, v3.'
    # The media is processed as usual...
    assert client.chunked_upload.called
    assert [call.kwargs['data']['names'] for call in api_calls(client, 'medias/resources-delete/')] == [
        'old_360.mp4'
    ]
    # ... and the resources of the media sharing the same files are refreshed too.
    refreshed = [call.kwargs['data']['oid'] for call in api_calls(client, 'medias/resources-check/')]
    assert refreshed == ['v1', 'v1', 'v2', 'v3']
    assert any('shared with 2 other media' in warning for warning in summary.warnings)


def test_process_media_shared_resources_dry_run(tmp_path):
    video = write(tmp_path / 'fa5381aa.mp4', b'x' * 100)
    client = make_client(
    ...
    assert status == 'planned'
    assert detail == '1 resource(s) to delete. Resources shared with: v2.'
    assert not api_calls(client, 'medias/resources-check/')


def test_process_media_ignores_refresh_error_of_a_shared_media(tmp_path):
    video = write(tmp_path / 'fa5381aa.mp4', b'x' * 100)
    client = make_client(
        resources_list=[
            [resource('old_360.mp4')],
    ...
    # The refresh of the shared media fails (deleted media for example), the run must go on.
    calls = {'count': 0}
    initial_api = client.api.side_effect

    def api(url, **kwargs):
        if url == 'medias/resources-check/' and (kwargs.get('data') or {}).get('oid') == 'v2':
            calls['count'] += 1
            raise ro.NudgisRequestError('HTTP 403 error on "url": Permission denied')
        return initial_api(url, **kwargs)
    ...
    assert any('Failed to refresh the resources' in warning for warning in summary.warnings)
